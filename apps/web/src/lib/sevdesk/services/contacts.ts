...
export class SevdeskContactsService {
    ...
    async createOrganizerWithContacts(input: CreateOrganizerContactInput) {
    ...
    async updateOrganizerContact(sevdeskCustomerId: string, input: CreateOrganizerContactInput) {
        const parsedInput = CreateOrganizerContactInputSchema.parse(input);
        ...
        return this.client
            .put(`/Contact/${sevdeskCustomerId}`, SevdeskCreateContactResponseSchema, companyPayload)
            .then((response) => response.objects);
    }
}
